...
- **Key Learnings**:
  - ❌ *Parcelflow Slow Browser UI & Timeout*: Running Playwright browser automation sequentially across 16 branches took 10+ mins and caused timeouts. Solution: Implemented Hybrid Direct REST API via `httpx`. Authenticate with NextAuth (`/api/auth/callback/credentials`), get Bearer JWT token (`/api/auth/session`), and query `/backend/delivery/epod/details` with combined vendor IDs (`89759` Shopee, `191226,191227,191228` Tiktok, `77` Lazada). Automatically falls back to Playwright if API fails.
